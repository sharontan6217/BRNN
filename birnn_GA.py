...
class neuralNetwork():
    ...
    def train(self):
       ...
       #generator
        ...
        x = np.zeros(shape=(PopulationSize, 1))
        x_mean = np.zeros(shape=PopulationSize)
        for i in range (PopulationSize):
            x[i, :] = x_predict[i]
        ...
        plot = ggplot(pd.melt(pd.concat([pd.DataFrame(x_mean, columns=["BiRNN Libor Distribution"]).reset_index(drop=True),
                                         pd.DataFrame(act_mean, columns=["Actual Libor Distribution"]).reset_index(drop=True)],
                                        axis=1))) + \
        ...
        xlab("Libor") + \
        ylab("Density") + \
        ggtitle("Trained Recurrent Neural Network (RNN) Predicted Libor Values") + \
        theme_matplotlib()
        plot.save(filename='output_ga_'+str(object=predictDate)+'_'+str(object=currentTime)+'birnn_ga_distribution.png', path=graph_dir_1)
